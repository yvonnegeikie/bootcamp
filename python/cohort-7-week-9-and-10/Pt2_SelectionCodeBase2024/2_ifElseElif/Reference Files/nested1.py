...
if userAge > ageLimit:
    userCode = input("Enter the usercode: ")

    # compare usercode with passcode 
    if userCode == passCode:
        # when usercode is the same as passcode 
        print("Code is valid, Access granted")
    else:
        print("Code is invalid, Access denied")
elif userAge == ageLimit:
    print(f"You have to be older than {userAge} to use the passcode")

#This else block will execute only if the userage is less than the agelimit
else:
    print("You have not met the age limit")
